...
/*******************************************************************************
ALL ARRAYS ARE INTEGER ARRAYS. DOUBLES TAKE DOUBLE THE SPACE, THE FIRST PART IS
ONE INTEGER THE SECOND PART IS A SECOND INTEGER. STRINGS ARE HOISTED AND DECLARED
INT THE DATA SECTION, A STRING ARRAY IS JUST AN ARRAY OF MEMORY ADDRESSES OF WHERE
THE STRINGS IN THE ARRAY ARE. OBJECTS ARE JUST ARRAYS.
*******************************************************************************/
...
public class SecondPass extends DepthFirstAdapter {
	...
	private String temp;
        ...
        private boolean[] argRegStatus = {false , false , false , false} ;
	...
	public void caseATypevarliststmtClassmethodstmt(ATypevarliststmtClassmethodstmt node) {
		...
		node.getId().apply(this);
                ...
                if(id.equals("MAIN"))
                {
                    //pull main to the top of the code part
                    tempCode = codePart ;
                    codePart = "" ;
                }
		...
		node.getLcurly().apply(this);
                ...
                codePart = codePart + id + ":\n" ;
                
                Method m = symbolTable.getMethod(id) ;
                HashMap vars = m.getAllVars() ;
 ...
 /*               
                ...
                if(id.equals("MAIN"))
                {
                    ...
                    codePart = codePart + "li $v0 , 10 \n" ;
                    codePart = codePart + "syscall \n" ;
                    codePart = codePart + tempCode ;
                }
                else
                {
                    popEverything() ;
                    codePart = codePart + "jr $ra \n" ;
                }
	...
	public void caseAIdvarlisttwoStmt(AIdvarlisttwoStmt node){
            ...
            //Varlisttwo handling should put things on stack.
            node.getVarlisttwo().apply(this);
            
            node.getRparen().apply(this);
            node.getSemicolon().apply(this);
            
            codePart = codePart + "jal " + id + "\n" ;
        ...
        //CLASS METHOD CALL ON AN OBJECT
	...
	public void caseAReturnStmt(AReturnStmt node){
            ...
            codePart = codePart + "move $a" + lastArgReg + " , $s" + lastOutReg + "\n" ;
            freeOutReg(lastOutReg) ;
	...
	public void caseAMultipleVarlisttwo(AMultipleVarlisttwo node){
            ...
            //FIND ME 2
            codePart = codePart + "move $a" + lastArgReg + " , $s" + lastOutReg + "\n" ;
            lastArgReg = getOpenArgReg(lastArgReg) ;
	...
	public void caseAIdvarlisttwoFactor(AIdvarlisttwoFactor node) {
            ...
            //Should put all the arguments on the stack
            node.getVarlisttwo().apply(this);
            node.getRparen().apply(this);
    ...
    public int getOpenArgReg(int reg)
    {
        int count = 0 ; 
        if(!argRegStatus[reg])
        {
            argRegStatus[reg] = true ;
            return reg ;
        }
        else
        {
            while(argRegStatus[reg])
            {
                reg++ ;
                if(reg > 7)
                {
    ...
    public void freeArgReg(int reg)
    {
        if(reg > 3)
        {
            System.out.println("REGISTER ERROR") ;
    ...
    public void freeAllReg()
    {
        ...
        for(int j = 0 ; j < argRegStatus.length ; j++)
        {
            regStatus[j] = false ;
        }
    }
    
    //method that puts the return address on the stack.
    public void stackRA()
    {
        codePart = codePart + "addi $sp , $sp , -4 \n" ;
        codePart = codePart + "sw $ra , 0($sp) \n" ;
    }

    //method gets the return address off the stack
    public void popRA()
    {
        codePart = codePart + "addi $sp , $sp , 4 \n" ;
        codePart = codePart + "lw $ra , 0($sp) \n" ;
    }
    
    //sets the frame pointer equal to the stack pointer
    public void setFP()
    {
        codePart = codePart + "move $fp , $sp \n" ;
    }
    
    public void stackT(int reg)
    {
        codePart = codePart + "addi $sp , $sp , -4 \n" ;
        codePart = codePart + "sw $t" + reg + " , 0($sp) \n" ;
    }
    
    public void stackS(int reg)
    {
        codePart = codePart + "addi $sp , $sp , -4 \n" ;
        codePart = codePart + "sw $s" + reg + " , 0($sp) \n" ;
    }
    
    public void stackA(int reg)
    {
        codePart = codePart + "addi $sp , $sp , -4 \n" ;
        codePart = codePart + "sw $a" + reg + " , 0($sp) \n" ;
    }
    
    public void popT(int reg)
    {
        codePart = codePart + "lw $t" + reg + " , 0($sp) \n" ;
        codePart = codePart + "addi $sp , $sp , 4 \n" ;
    }
    
    public void popS(int reg)
    {
        codePart = codePart + "lw $s" + reg + " , 0($sp) \n" ;
        codePart = codePart + "addi $sp , $sp , 4 \n" ;
    }
    
    public void popA(int reg)
    {
        codePart = codePart + "lw $a" + reg + " , 0($sp) \n" ;
        codePart = codePart + "addi $sp , $sp , 4 \n" ;
    }
    
    public void stackEverything()
    {
        stackRA() ;
        setFP() ;
        for(int i = 0 ; i < 8 ; i++)
